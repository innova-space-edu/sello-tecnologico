...
export default function EvidenciasPage() {
  ...
  const fetchEvidencias = async (userRole: string, uid: string) => {
    ...
    // Estudiantes solo ven sus propias evidencias
                  ...
                  <div className="flex flex-col gap-1 shrink-0">
                    {(ev.created_by === userId || !esEstudiante) && (
                      <Link href={`/evidencias/${ev.id}/editar`}
                        className="text-blue-400 hover:text-blue-600 hover:bg-blue-50 p-1.5 rounded-lg transition-colors text-xs text-center"
                        title="Editar">
                        ✏️
                      </Link>
                    )}
