...
import DeletePublicPageButton from '@/components/vitrinas/DeletePublicPageButton'
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-16 lg:pt-8">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            <p className="text-xs uppercase tracking-widest text-sky-500 font-semibold">Páginas de Proyectos</p>
        ...
        <section className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          {pages.length > 0 ? (
            <div className="divide-y divide-gray-100">
              {pages.map(page => {
                const projectId = page.project_id ?? page.projects?.id
                const pageTitle = cleanTitle(page.title)
                return (
                  <div key={page.id} className="px-5 lg:px-6 py-4 hover:bg-blue-50 transition-colors">
                    <div className="flex flex-wrap justify-between gap-3 items-start">
                      <div className="min-w-0">
                        ...
                        <h3 className="font-semibold text-gray-800 truncate">{pageTitle}</h3>
                      ...
                      <div className="flex flex-wrap gap-2 shrink-0">
                        {projectId ? (
                          <Link href={`/proyectos/${projectId}/pagina`} className="text-sm bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold">
                            Editar
                          </Link>
                        ) : (
                          <span className="text-sm bg-gray-100 text-gray-400 px-4 py-2 rounded-lg font-semibold cursor-not-allowed">
                            Sin proyecto
                          </span>
                        )}
                        {page.is_public && (
                        ...
                        <DeletePublicPageButton pageId={page.id} title={pageTitle} />
                      </div>
                    </div>
                  </div>
                )
