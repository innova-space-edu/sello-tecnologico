  ...
  const fetchProyectos = async (userRole: string, uid: string) => {
    ...
    if (userRole === 'estudiante') {
      const [{ data: propios }, { data: colaboraciones }] = await Promise.all([
        supabase.from('projects').select(baseSelect).eq('owner_id', uid).order('created_at', { ascending: false }),
        supabase.from('project_collaborators').select('project_id').eq('user_id', uid).eq('status', 'accepted'),
      ])

      const ids = Array.from(new Set((colaboraciones ?? []).map((c: any) => c.project_id).filter(Boolean)))
      let compartidos: any[] = []
      if (ids.length > 0) {
        const { data } = await supabase.from('projects').select(baseSelect).in('id', ids).order('created_at', { ascending: false })
        compartidos = data ?? []
      }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      channel = supabase.channel('proyectos-realtime')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'projects' }, () => fetchProyectos(rolRef.current, userIdRef.current))
        .on('postgres_changes', { event: '*', schema: 'public', table: 'project_collaborators' }, () => fetchProyectos(rolRef.current, userIdRef.current))
        .subscribe()
    }
    init()
      ...
      <main className="lg:ml-64 flex-1 p-4 lg:p-8 pt-16 lg:pt-8">
        ...
        {cursoFiltro && (
          <div className="bg-blue-50 border border-blue-100 text-blue-800 rounded-xl px-4 py-3 mb-4 flex flex-wrap justify-between gap-3 items-center text-sm">
            ...
            <Link href="/proyectos" className="font-semibold hover:underline">Ver todos</Link>
          </div>
        )}
