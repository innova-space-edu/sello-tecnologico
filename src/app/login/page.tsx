...
export default function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { error: profileError } = await supabase.from('profiles').upsert({
      id: data.user.id, email, full_name: form.full_name.trim(), rut, curso, role,
    })
    if (profileError) {
      setError('La cuenta se creó, pero no se pudo guardar el perfil. Avísame y lo corregimos.')
      setLoading(false)
      return
    }
              ...
              <div>
                ...
                {form.email && (
                  <p className="text-xs mt-1 font-medium"
                    style={{ color: form.email.trim().toLowerCase().endsWith('@colprovidencia.cl') ? '#15803d' : '#2563eb' }}>
