...
import MessageContent from '@/components/mensajes/MessageContent'
...
export default function MensajesPage() {
    ...
    const init = async () => {
      ...
      if (perfil.role === 'estudiante') {
        ...
        if (perfil.curso) {
          ...
          visibles = [...(companeros ?? []), ...(docentes ?? [])].filter(usuario => {
            if (ids.has(usuario.id)) return false
            ids.add(usuario.id)
            return true
          })
        } else {
      ...
      presenceChannel
        .on('presence', { event: 'sync' }, () => {
          ...
          const ids = new Set(Object.values(state).flat().map((presence: any) => presence.user_id))
          setOnlineUsers(ids)
        })
        .on('presence', { event: 'join' }, ({ newPresences }: any) => {
          setOnlineUsers(prev => {
            const next = new Set(prev)
            newPresences.forEach((presence: any) => next.add(presence.user_id))
            return next
          })
        })
        .on('presence', { event: 'leave' }, ({ leftPresences }: any) => {
          setOnlineUsers(prev => {
            const next = new Set(prev)
            leftPresences.forEach((presence: any) => next.delete(presence.user_id))
            return next
          })
        })
  ...
  const handleSend = async (event: React.FormEvent) => {
    ...
    if (!input.trim() || !selectedUser || !currentUser || sending) return
    ...
    if (!response.ok || data.status === 'held_for_review') {
      setWarning(data.warning ?? data.error ?? 'El mensaje no pudo ser enviado.')

      if (data.status === 'blocked') {
        setBloqueado(true)
        setTimeout(() => router.push('/bloqueado'), 2500)
      }

      if (String(data.error ?? '').toLowerCase().includes('conversación')) {
    ...
    return (
      <div className="fixed inset-0 bg-red-900 bg-opacity-95 flex items-center justify-center z-50">
        <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full text-center mx-4">
          ...
          <p className="text-gray-600 mb-4">Tu cuenta fue bloqueada por seguridad y el administrador fue notificado.</p>
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
            <p className="text-sm text-red-700 font-medium">🚨 El administrador revisará el caso y tomará las medidas correspondientes.</p>
          </div>
            ...
            {usuarios.map(usuario => (
              <button
                ...
                className={`w-full px-5 py-3.5 flex items-center gap-3 hover:bg-blue-50 transition-colors text-left border-b border-gray-50 ${
                  selectedUser?.id === usuario.id ? 'bg-blue-50 border-l-4 border-l-blue-600' : ''
                }`}
              >
                <div className="relative shrink-0">
                  ...
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${isOnline(usuario.id) ? 'bg-green-500' : 'bg-red-400'}`} />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center">
                    <p className="font-medium text-gray-800 text-sm truncate">{usuario.full_name ?? usuario.email}</p>
                    {(unread[usuario.id] ?? 0) > 0 && (
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${roleColor[String(usuario.role)] ?? 'bg-gray-100 text-gray-500'}`}>
                      {usuario.role ?? 'usuario'}
                    </span>
                    <span className={`text-xs ${isOnline(usuario.id) ? 'text-green-500' : 'text-gray-400'}`}>
                      {isOnline(usuario.id) ? '● En línea' : '● Desconectado'}
                    </span>
                  </div>
                </div>
              </button>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="bg-white px-6 py-4 border-b border-gray-200 flex items-center gap-3 shrink-0">
              <div className="relative">
                ...
                <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${isOnline(selectedUser.id) ? 'bg-green-500' : 'bg-red-400'}`} />
              </div>
              ...
              {parBloqueado && (
                <span className="text-xs bg-red-100 text-red-600 px-3 py-1 rounded-full font-medium">🚫 Conversación bloqueada</span>
              )}
            </div>
            ...
            {parBloqueado && (
              <div className="mx-4 mt-4 bg-red-50 border border-red-300 rounded-xl p-4 flex items-center gap-3 shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 mt-0.5">No puedes enviar ni recibir mensajes de este usuario.</p>
                </div>
              </div>
            )}

            {warning && !parBloqueado && (
              <div className="mx-4 mt-4 bg-amber-50 border border-amber-300 rounded-xl p-4 flex items-start gap-3 shrink-0">
                <span className="text-2xl shrink-0">⚠️</span>
                <div className="flex-1">
                  <p className="text-sm font-bold text-amber-700 mb-1">Aviso de seguridad</p>
                  <p className="text-xs text-amber-700">{warning}</p>
                </div>
                <button onClick={() => setWarning('')} className="text-amber-500 hover:text-amber-700 shrink-0">✕</button>
              </div>
            )}
                  ...
                  <div key={message.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-sm px-4 py-2.5 rounded-2xl text-sm ${
                      ...
                      <p className={`text-xs mt-1 ${isMe ? 'text-blue-200' : 'text-gray-400'}`}>
                        {new Date(message.created_at).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}
            ...
            <div className="bg-white px-6 py-4 border-t border-gray-200 shrink-0">
              {parBloqueado ? (
                ...
                <form onSubmit={handleSend} className="flex gap-3">
                  <input
                    value={input}
                    onChange={event => { setInput(event.target.value); setWarning('') }}
              ...
              <p className="text-xs text-gray-400 mt-2 text-center">Los mensajes son monitoreados para garantizar un entorno escolar seguro</p>
            </div>
          </div>
        ) : (
