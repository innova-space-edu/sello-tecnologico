...
function cellWithShift(cell: GridCell, rowDelta: number, colDelta: number): GridCell {
  ...
  if (next.formula) {
    ...
    next.value = `=${shiftFormula(next.value.slice(1), rowDelta, colDelta)}`
    next.formula = String(next.value).slice(1)
    next.cached = undefined
  }
  return next
}

export default function NativeSpreadsheetRanges({ workbookId, title }: Props) {
  ...
  const [selected, setSelected] = useState<Point>({ row: 0, col: 0 })
  ...
  const beginSelection = (row: number, col: number, extend = false) => {
    if (!sheet) return
  ...
  const selectAll = () => {
    ...
    setSelected({ row: Math.max(0, sheet.cells.length - 1), col: Math.max(0, (sheet.cells[0]?.length ?? 1) - 1) })
    syncFormulaBar(0, 0)
  }
  ...
  const pasteSelection = () => {
    ...
    if (!clip || !sheet) {
      setNotice('Primero copia o corta un rango dentro de Promedios.')
      return
    }
    ...
    mutateSheet((target) => {
      ...
      if (clip.cut) {
        cells = cells.map((row, rowIndex) => row.map((cell, colIndex) => insideRange(clip.range, rowIndex, colIndex) ? emptyCell() : cell))
      }
    ...
    setSelected({ row: Math.min(sheet.cells.length - 1, destination.row + clip.cells.length - 1), col: Math.min((sheet.cells[0]?.length ?? 1) - 1, destination.col + (clip.cells[0]?.length ?? 1) - 1) })
    setNotice('Rango pegado. Las referencias relativas de las fórmulas fueron ajustadas.')
  }
  ...
  const pasteFormat = () => {
    ...
    if (!formats?.length) {
      setNotice('Primero usa Copiar formato.')
      return
    }
    mutateSheet((target) => ({
      ...target,
  ...
  const fillDown = () => {
    ...
    setNotice('Relleno hacia abajo aplicado; las fórmulas relativas se ajustaron.')
  }
  ...
  const fillRight = () => {
    ...
    setNotice('Relleno hacia la derecha aplicado; las fórmulas relativas se ajustaron.')
  }
  ...
  const createBlank = () => {
    ...
    setActiveSheet(0); setAnchor({ row: 0, col: 0 }); setSelected({ row: 0, col: 0 }); setFormulaBar('')
    setSourceName('Planilla nueva'); setError(''); setNotice('Planilla creada. Se guardará solo cuando presiones Guardar.')
  }
  ...
  const addSheet = () => {
    ...
    setActiveSheet(next); setAnchor({ row: 0, col: 0 }); setSelected({ row: 0, col: 0 }); setFormulaBar('')
  }
  ...
  const loadBuffer = async (buffer: ArrayBuffer, label: string) => {
    ...
    try {
      ...
      setWorkbook(next); setActiveSheet(0); setAnchor({ row: 0, col: 0 }); setSelected({ row: 0, col: 0 }); setFormulaBar(''); setSourceName(label)
  ...
  const exportPdf = () => {
    ...
    doc.setFontSize(14); doc.text(`${title} — ${sheet.name}`, 14, 16)
  ...
  const addCourseSheet = async () => {
    ...
    try {
      ...
      ;['Estudiante', 'Avance 1', 'Avance 2', 'Avance 3', '% Final', 'Nota', 'Estado'].forEach((label, col) => { target.cells[0][col] = { value: label, style: { bold: true, background: '#F1F5F9', horizontal: 'center', border: true } } })
      ...
      setActiveSheet(currentCount); setAnchor({ row: 0, col: 0 }); setSelected({ row: 0, col: 0 }); setNotice(`Hoja “${target.name}” creada con ${students.length} estudiantes.`)
  ...
  const insertImage = async (file?: File) => {
    ...
    try {
      ...
      mutateSheet((target) => ({ ...target, cells: target.cells.map((row, rowIndex) => row.map((cell, colIndex) => rowIndex === selection.startRow && colIndex === selection.startCol ? { ...cell, image } : cell)) }))
  ...
  const insertLink = () => {
    ...
    mutateSheet((target) => ({ ...target, cells: target.cells.map((row, rowIndex) => row.map((cell, colIndex) => insideRange(selection, rowIndex, colIndex) ? { ...cell, hyperlink: url, value: cell.value || url } : cell)) }))
  ...
  const insertQr = async () => {
    ...
    try {
      ...
      mutateSheet((target) => ({ ...target, cells: target.cells.map((row, rowIndex) => row.map((cell, colIndex) => rowIndex === selection.startRow && colIndex === selection.startCol ? { ...cell, image, value: cell.value || 'QR' } : cell)) }))
  ...
  const addChart = () => {
    if (!sheet) return
    mutateSheet((target) => ({ ...target, charts: [...target.charts, { id: `${Date.now()}`, kind: chartKind, title: `Gráfico ${target.charts.length + 1}`, range: selection }] }))
    setShowChartPanel(true); setNotice('Gráfico creado desde el rango seleccionado.')
  }
  ...
  const moveAfterEdit = (row: number, col: number, direction: 'down' | 'right' | 'left') => {
    ...
    setAnchor(next); setSelected(next); syncFormulaBar(next.row, next.col)
  }
    ...
    <div className="flex min-h-screen min-w-0 flex-1 flex-col bg-slate-100">
      ...
      <div className="sticky top-0 z-30 border-b border-slate-300 bg-white shadow-sm">
        <div className="flex items-center gap-3 px-4 py-2 sm:px-6">
          ...
          <div className="min-w-0 flex-1"><div className="truncate text-sm font-black text-slate-900">{title}</div><div className="text-[10px] font-semibold text-slate-500">Editor nativo · selección múltiple · Supabase</div></div>
      ...
      {!workbook ? <div className="flex flex-1 items-center justify-center p-6"><div className="w-full max-w-2xl rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm"><div className="text-5xl">📊</div><h2 className="mt-4 text-2xl font-black text-slate-900">Promedios</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600">Editor propio del Sello Tecnológico con selección y edición de rangos.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><button onClick={() => fileInputRef.current?.click()} className="rounded-2xl bg-blue-700 px-6 py-3 text-sm font-black text-white">↑ Abrir Excel</button><button onClick={() => void openSaved()} className="rounded-2xl border border-slate-300 px-6 py-3 text-sm font-black text-slate-700">📁 Archivos guardados</button><button onClick={createBlank} className="rounded-2xl border border-slate-300 px-6 py-3 text-sm font-black text-slate-700">+ Crear vacía</button></div></div></div> : sheet ? <>
        <div className="border-b border-slate-200 bg-white px-4 py-2 sm:px-6">
          <div className="flex items-center gap-2"><div className="min-w-28 rounded-lg border border-slate-300 bg-slate-50 px-2 py-2 text-center text-xs font-black text-slate-600">{rangeAddress(selection)}</div><span className="rounded bg-blue-50 px-2 py-1 text-[10px] font-black text-blue-700">{selectionCount} celda{selectionCount === 1 ? '' : 's'}</span><span className="text-xs font-black text-slate-500">fx</span><input value={formulaBar} onChange={(event) => setFormulaBar(event.target.value)} onBlur={() => { if (!multiSelection) updateCell(selected.row, selected.col, formulaBar) }} onKeyDown={(event) => { if (event.key === 'Enter' && !multiSelection) { updateCell(selected.row, selected.col, formulaBar); event.currentTarget.blur() } }} disabled={multiSelection} className="min-w-0 flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 disabled:bg-slate-50 disabled:text-slate-400" placeholder={multiSelection ? 'Varias celdas seleccionadas: usa las herramientas de rango' : 'Valor o fórmula, ej. =PROMEDIO(B2:D2)'} /></div>
          <div className="mt-1 text-[10px] text-slate-500">Arrastra para seleccionar varias celdas · Shift + clic extiende · clic en A/B/C selecciona columna · clic en 1/2/3 selecciona fila.</div>
        </div>

        {multiSelection && <div className="sticky top-[126px] z-20 flex items-center gap-1 overflow-x-auto border-b border-blue-200 bg-blue-50 px-3 py-2 shadow-sm"><span className="mr-2 whitespace-nowrap text-[11px] font-black text-blue-800">Rango {rangeAddress(selection)} · {selectionRows}×{selectionCols}</span><button className={contextClass} onClick={() => copySelection(false)}>Copiar</button><button className={contextClass} onClick={() => copySelection(true)}>Cortar</button><button className={contextClass} onClick={pasteSelection}>Pegar</button><button className={contextClass} onClick={() => toggleStyle('bold')}>Negrita</button><button className={contextClass} onClick={() => applyStyle({ horizontal: 'center' })}>Centrar</button><button className={contextClass} onClick={() => toggleStyle('border')}>Bordes</button><button className={contextClass} onClick={mergeSelection}>Combinar</button><button className={contextClass} onClick={fillDown}>↓ Rellenar</button><button className={contextClass} onClick={fillRight}>→ Rellenar</button><button className={contextClass} onClick={copyFormat}>Copiar formato</button><button className={contextClass} onClick={pasteFormat}>Aplicar formato</button><button className={contextClass} onClick={clearContents}>Borrar contenido</button><button className={contextClass} onClick={clearFormats}>Borrar formato</button></div>}

        <div className="flex-1 overflow-auto bg-white"><div style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top left', width: `${10000 / zoom}%` }}><table className="min-w-max border-collapse text-sm select-none">
          <thead className="sticky top-0 z-20 bg-slate-100"><tr><th onClick={selectAll} className="h-8 w-12 cursor-pointer border border-slate-300 bg-slate-200 text-[10px] font-black text-slate-600 hover:bg-blue-100">▣</th>{sheet.cells[0].map((_, index) => <th key={index} onMouseDown={(event) => { event.preventDefault(); selectColumn(index) }} className={`h-8 cursor-pointer border border-slate-300 px-2 text-center font-black ${selection.startCol <= index && index <= selection.endCol ? 'bg-blue-100 text-blue-800' : 'text-slate-700 hover:bg-slate-200'}`} style={{ minWidth: `${Math.max(72, sheet.columnWidths[index] * 7)}px` }}>{columnLabel(index)}</th>)}</tr></thead>
          <tbody>{sheet.cells.map((row, rowIndex) => <tr key={rowIndex} style={{ height: `${sheet.rowHeights[rowIndex]}px` }}><th onMouseDown={(event) => { event.preventDefault(); selectRow(rowIndex) }} className={`sticky left-0 z-10 w-12 cursor-pointer border border-slate-300 text-center text-xs font-black ${selection.startRow <= rowIndex && rowIndex <= selection.endRow ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{rowIndex + 1}</th>{row.map((cell, colIndex) => {
            ...
            return <td key={`${rowIndex}-${colIndex}`} colSpan={colSpan} rowSpan={rowSpan} onMouseDown={(event) => { if (event.button !== 0) return; beginSelection(rowIndex, colIndex, event.shiftKey) }} onMouseEnter={() => extendSelection(rowIndex, colIndex)} className={`${sheet.showGridlines || cell.style?.border ? 'border border-slate-300' : 'border border-transparent'} relative p-0 ${inSelection ? 'bg-blue-50/40 ring-1 ring-inset ring-blue-400' : ''} ${active ? 'ring-2 ring-inset ring-blue-600' : ''}`} style={{ minWidth: `${width}px`, ...renderStyle(cell) }}>
              ...
              <input value={displayed[rowIndex]?.[colIndex] ?? ''} onFocus={() => { if (!dragSelecting && !multiSelection) syncFormulaBar(rowIndex, colIndex) }} onChange={(event) => { if (multiSelection) return; updateCell(rowIndex, colIndex, event.target.value); setFormulaBar(event.target.value) }} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); moveAfterEdit(rowIndex, colIndex, 'down') } else if (event.key === 'Tab') { event.preventDefault(); moveAfterEdit(rowIndex, colIndex, event.shiftKey ? 'left' : 'right') } }} readOnly={multiSelection && !active} draggable={false} className="h-full min-h-9 w-full border-0 bg-transparent px-2 outline-none" style={{ ...renderStyle(cell), minWidth: `${width}px`, paddingTop: cell.image ? '58px' : undefined }} />
