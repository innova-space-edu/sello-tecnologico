...
function emptyCell(): GridCell { return { value: '' } }
...
function fromExcelCell(cell: ExcelCell): GridCell {
  ...
  if (cell.formula) {
    return { value: `=${cell.formula}`, formula: cell.formula, cached: cell.result == null ? undefined : toPrimitive(cell.result), style }
  }
  ...
  while (changed && expression.startsWith('(') && expression.endsWith(')')) {
    ...
    for (let i = 0; i < expression.length; i += 1) {
      ...
      if ((char === '"' || char === "'") && expression[i - 1] !== '\\') {
        if (!quote) quote = char; else if (quote === char) quote = ''
      } else if (!quote) {
        if (char === '(') depth += 1
        else if (char === ')') depth -= 1
    ...
    if (depth === 0) { expression = expression.slice(1, -1).trim(); changed = true }
  }
  return expression
}

function findTopLevelComparison(expression: string) {
  let depth = 0; let quote = ''
  for (let i = 0; i < expression.length; i += 1) {
    const char = expression[i]
    if ((char === '"' || char === "'") && expression[i - 1] !== '\\') {
    ...
    if (['>=', '<=', '<>'].includes(two)) return { index: i, operator: two }
    if (['=', '>', '<'].includes(char)) return { index: i, operator: char }
  }
  return null
}

function findTopLevelBinary(expression: string, operators: string[]) {
  let depth = 0; let quote = ''; let candidate: { index: number; operator: string } | null = null
  for (let i = 0; i < expression.length; i += 1) {
    const char = expression[i]
    if ((char === '"' || char === "'") && expression[i - 1] !== '\\') {
      if (!quote) quote = char; else if (quote === char) quote = ''
    ...
    if ((char === '+' || char === '-') && (i === 0 || '+-*/^(,<>=;'.includes(expression[i - 1]))) continue
    candidate = { index: i, operator: char }
  }
  return candidate
}

function parseWholeFunction(expression: string) {
  const match = expression.match(/^([A-ZÁÉÍÓÚ.]+)\s*\(/i)
  if (!match) return null
  ...
  for (let i = open; i < expression.length; i += 1) {
    ...
    if (char === '(') depth += 1
      ...
      if (depth === 0) {
        ...
        return { name: match[1].toUpperCase(), args: splitArgs(expression.slice(open + 1, i)) }
      }
    }
  }
  return null
...
function rangeValues(sheet: SheetData, token: string, visited: Set<string>) {
  const parsed = parseRange(stripOuterParens(token.replace(/\$/g, '')))
  if (!parsed) return []
...
function evaluateExpression(sheet: SheetData, raw: string, visited: Set<string>): Primitive | undefined {
  let expression = stripOuterParens(raw.trim().replace(/^=/, ''))
  ...
  if (comparison) {
    const left = evaluateExpression(sheet, expression.slice(0, comparison.index), new Set(visited))
    const right = evaluateExpression(sheet, expression.slice(comparison.index + comparison.operator.length), new Set(visited))
    return compareValues(left, right, comparison.operator)
  }

  const addSub = findTopLevelBinary(expression, ['+', '-'])
  if (addSub) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, addSub.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(addSub.index + 1), new Set(visited)))
    return addSub.operator === '+' ? left + right : left - right
  }

  const mulDiv = findTopLevelBinary(expression, ['*', '/'])
  if (mulDiv) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, mulDiv.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(mulDiv.index + 1), new Set(visited)))
    if (mulDiv.operator === '/' && right === 0) return undefined
    return mulDiv.operator === '*' ? left * right : left / right
  }

  const power = findTopLevelBinary(expression, ['^'])
  if (power) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, power.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(power.index + 1), new Set(visited)))
    return left ** right
  }

  if (expression.startsWith('+')) return toNumber(evaluateExpression(sheet, expression.slice(1), new Set(visited)))
  if (expression.startsWith('-')) return -toNumber(evaluateExpression(sheet, expression.slice(1), new Set(visited)))

  const ref = parseCellRef(expression)
  if (ref) return computeCell(sheet, ref.row, ref.col, new Set(visited))

  const call = parseWholeFunction(expression)
  if (!call) return undefined
  const { name, args } = call

  if (name === 'SI' || name === 'IF') {
    const condition = evaluateExpression(sheet, args[0] ?? '', new Set(visited))
    return evaluateExpression(sheet, Boolean(condition) ? (args[1] ?? '""') : (args[2] ?? '""'), new Set(visited))
  }
  ...
  if (name === 'Y' || name === 'AND') return args.every((arg) => Boolean(evaluateExpression(sheet, arg, new Set(visited))))
  if (name === 'O' || name === 'OR') return args.some((arg) => Boolean(evaluateExpression(sheet, arg, new Set(visited))))
  ...
  const values = args.flatMap((arg) => {
    const clean = stripOuterParens(arg)
    return clean.includes(':') && parseRange(clean.replace(/\$/g, '')) ? rangeValues(sheet, clean, visited) : [evaluateExpression(sheet, clean, new Set(visited))]
...
function computeCell(sheet: SheetData, row: number, col: number, visited = new Set<string>()): Primitive {
  ...
  if (calculated !== undefined) return typeof calculated === 'number' ? Math.round(calculated * 1000000) / 1000000 : calculated
...
async function ensureExcelJS() {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    if (existing) {
      if (browser.ExcelJS) { resolve(); return }
      existing.addEventListener('load', () => resolve(), { once: true })
      existing.addEventListener('error', () => reject(new Error('No se pudo cargar el motor XLSX')), { once: true })
      return
    }
    const script = document.createElement('script')
    script.src = EXCELJS_SRC; script.async = true
    script.onload = () => resolve(); script.onerror = () => reject(new Error('No se pudo cargar el motor XLSX'))
...
function excelFormula(raw: string) {
  ...
  formula = formula
    .replace(/\bSUMA\(/gi, 'SUM(').replace(/\bPROMEDIO\(/gi, 'AVERAGE(')
    .replace(/\bCONTAR\.SI\(/gi, 'COUNTIF(').replace(/\bCONTARA\(/gi, 'COUNTA(').replace(/\bCONTAR\(/gi, 'COUNT(')
    .replace(/\bSUMAPRODUCTO\(/gi, 'SUMPRODUCT(').replace(/\bREDONDEAR\(/gi, 'ROUND(')
    .replace(/\bSI\.ERROR\(/gi, 'IFERROR(').replace(/\bSI\(/gi, 'IF(')
    .replace(/\bY\(/gi, 'AND(').replace(/\bO\(/gi, 'OR(').replace(/;/g, ',')
...
function applyExcelStyle(cell: ExcelCell, style?: CellStyle) {
  if (!style) return
  if (style.bold || style.italic || style.color) cell.font = { bold: style.bold, italic: style.italic, color: style.color ? { argb: `FF${style.color.replace('#', '').toUpperCase()}` } : undefined }
  if (style.background) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${style.background.replace('#', '').toUpperCase()}` } }
  if (style.horizontal || style.vertical || style.wrap) cell.alignment = { horizontal: style.horizontal, vertical: style.vertical, wrapText: style.wrap }
...
export default function LightSpreadsheet({ workbookId, title }: Props) {
  ...
  const loadBuffer = async (buffer: ArrayBuffer, label: string) => {
    ...
    try {
      ...
      const sheets: SheetData[] = xlsx.worksheets.map((ws, sheetIndex) => {
        ...
        return {
          name: ws.name || `Hoja ${sheetIndex + 1}`, cells, merges,
          columnWidths: Array.from({ length: cols }, (_, col) => Math.max(8, Math.min(50, ws.columns[col]?.width ?? (col === 0 ? 24 : 14)))),
          rowHeights: Array.from({ length: rows }, (_, row) => Math.max(18, Math.min(120, ws.getRow(row + 1).height ?? 22))),
        }
      })
      setWorkbook({ sheets: sheets.length ? sheets : [emptySheet()] })
      setActiveSheet(0); setSelected({ row: 0, col: 0 }); setFormulaBar(''); setSourceName(label)
      setNotice(`${sheets.length || 1} hoja${sheets.length === 1 ? '' : 's'} cargada${sheets.length === 1 ? '' : 's'}. Las fórmulas quedan conectadas y se recalculan al editar sus celdas.`)
    } catch (err) { setError(err instanceof Error ? err.message : 'No se pudo abrir el Excel') }
    finally { setOpening(false) }
  }

  const uploadLocal = async (file?: File) => {
    if (!file) return
    if (!file.name.toLowerCase().endsWith('.xlsx')) { setError('Selecciona un archivo .xlsx'); return }
  ...
  const openSaved = async () => {
    ...
    try {
      ...
      const buffer = await response.arrayBuffer(); setOpening(false); await loadBuffer(buffer, 'Archivo guardado en Supabase')
    } catch (err) { setOpening(false); setError(err instanceof Error ? err.message : 'No se pudo abrir el archivo guardado') }
  }
  ...
  const updateCell = (row: number, col: number, value: Primitive) => {
    ...
    setWorkbook((current) => {
      ...
      return { ...current, sheets: current.sheets.map((item, index) => index === activeSheet ? {
        ...item,
  ...
  const createBlank = () => {
    setWorkbook({ sheets: [emptySheet()] }); setActiveSheet(0); setSelected({ row: 0, col: 0 }); setFormulaBar('')
    setSourceName('Planilla nueva'); setError(''); setNotice('Planilla vacía creada. Nada se guardará hasta que presiones Guardar.')
  }

  const addSheet = () => {
    if (!workbook) { createBlank(); return }
  ...
  const addCourseSheet = async () => {
    if (!courseId) { setNotice('Selecciona un curso.'); return }
    setNotice(''); setError('')
    try {
      ...
      setActiveSheet(currentCount); setSourceName((current) => current || 'Planilla de curso')
  ...
  const save = async () => {
    ...
    try {
      const ExcelJS = await ensureExcelJS(); const xlsx = new ExcelJS.Workbook()
      workbook.sheets.forEach((source, sheetIndex) => {
        ...
        source.cells.forEach((row, rowIndex) => row.forEach((gridCell, colIndex) => {
          ...
          if (gridCell.formula || (typeof gridCell.value === 'string' && gridCell.value.startsWith('='))) {
            ...
            const currentResult = computeCell(source, rowIndex, colIndex)
            const result = typeof currentResult === 'string' && currentResult.startsWith('=') ? gridCell.cached : currentResult
            cell.value = result === undefined ? { formula } : { formula, result }
          } else cell.value = gridCell.value
      ...
      setNotice('Cambios guardados en Supabase con los resultados actuales de las fórmulas.')
  ...
  return (
    <div className="flex min-h-screen min-w-0 flex-1 flex-col bg-slate-100">
      ...
      <div className="border-b border-slate-200 bg-white px-4 py-3 shadow-sm sm:px-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><div className="truncate text-lg font-black text-slate-900">{title}</div><div className="text-xs text-slate-500">Editor de notas · fórmulas reactivas · guardado solo en Supabase</div></div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => fileInputRef.current?.click()} disabled={opening} className="rounded-xl bg-blue-700 px-4 py-2.5 text-sm font-black text-white hover:bg-blue-800 disabled:opacity-50">{opening ? 'Abriendo…' : '↑ Subir Excel'}</button>
            ...
            {workbook && <><button onClick={save} disabled={saving} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-50">{saving ? 'Guardando…' : 'Guardar'}</button><a href={`/api/promedios/${workbookId}/download`} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-black text-slate-700 hover:bg-slate-50">Descargar .xlsx</a></>}
          </div>
        </div>
        {workbook && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
      ...
      {!workbook ? <div className="flex flex-1 items-center justify-center p-6"><div className="w-full max-w-2xl rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
        <div className="text-5xl">📊</div><h2 className="mt-4 text-2xl font-black text-slate-900">La planilla comienza vacía</h2>
        <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600">Sube un Excel para cargar cada hoja como una pestaña independiente. Las fórmulas quedan conectadas a sus celdas y se recalculan cuando cambias una nota o porcentaje.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-3"><button onClick={() => fileInputRef.current?.click()} className="rounded-2xl bg-blue-700 px-6 py-3 text-sm font-black text-white hover:bg-blue-800">↑ Subir Excel</button><button onClick={openSaved} className="rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-black text-slate-700 hover:bg-slate-50">📁 Abrir desde Supabase</button><button onClick={createBlank} className="rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-black text-slate-700 hover:bg-slate-50">+ Crear vacía</button></div>
      </div></div> : sheet ? <>
        <div className="border-b border-slate-200 bg-white px-4 py-2 sm:px-6"><div className="flex items-center gap-2">
          ...
          <input value={formulaBar} onChange={(e) => setFormulaBar(e.target.value)} onBlur={() => updateCell(selected.row, selected.col, formulaBar)} onKeyDown={(e) => { if (e.key === 'Enter') { updateCell(selected.row, selected.col, formulaBar); (e.currentTarget as HTMLInputElement).blur() } }} className="min-w-0 flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500" placeholder="Valor o fórmula, ej. =PROMEDIO(B2:D2)" />
        </div><div className="mt-1 text-[11px] text-slate-500">Fórmulas conectadas: SUMA/SUM, PROMEDIO/AVERAGE, SUMAPRODUCTO/SUMPRODUCT, CONTAR/COUNT, CONTAR.SI/COUNTIF, SI/IF, REDONDEAR/ROUND, MIN, MAX, ABS, NOTA y operaciones + − × ÷.</div></div>
        ...
        <tbody>{sheet.cells.map((row, rowIndex) => <tr key={rowIndex} style={{ height: `${sheet.rowHeights[rowIndex]}px` }}><th className="sticky left-0 z-10 w-12 border border-slate-300 bg-slate-100 text-center text-xs font-black text-slate-600">{rowIndex + 1}</th>{row.map((cell, colIndex) => {
          ...
          const colSpan = mergeInfo ? mergeInfo.merge.endCol - mergeInfo.merge.startCol + 1 : 1
          const rowSpan = mergeInfo ? mergeInfo.merge.endRow - mergeInfo.merge.startRow + 1 : 1
          const width = mergeInfo ? sheet.columnWidths.slice(mergeInfo.merge.startCol, mergeInfo.merge.endCol + 1).reduce((sum, value) => sum + value * 7, 0) : Math.max(72, sheet.columnWidths[colIndex] * 7)
          return <td key={`${rowIndex}-${colIndex}`} colSpan={colSpan} rowSpan={rowSpan} className={`border border-slate-300 p-0 ${active ? 'ring-2 ring-inset ring-blue-500' : ''}`} style={{ minWidth: `${width}px`, ...renderStyle(cell) }} onClick={() => chooseCell(rowIndex, colIndex)}>
            <input value={displayed[rowIndex]?.[colIndex] ?? ''} onFocus={() => chooseCell(rowIndex, colIndex)} onChange={(e) => { updateCell(rowIndex, colIndex, e.target.value); setFormulaBar(e.target.value) }} className="h-full min-h-9 w-full border-0 bg-transparent px-2 outline-none" style={{ ...renderStyle(cell), minWidth: `${width}px` }} />
          </td>
        })}</tr>)}</tbody></table></div>

        <div className="flex items-center gap-2 overflow-x-auto border-t border-slate-200 bg-slate-100 px-4 py-2">{workbook.sheets.map((item, index) => <button key={`${item.name}-${index}`} onClick={() => { setActiveSheet(index); setSelected({ row: 0, col: 0 }); setFormulaBar('') }} className={`whitespace-nowrap rounded-lg px-3 py-2 text-xs font-black ${activeSheet === index ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-600 hover:bg-white/70'}`}>{item.name}</button>)}<button onClick={addSheet} className="rounded-lg border border-dashed border-slate-400 px-3 py-2 text-xs font-black text-slate-600">+ Hoja</button></div>
      </> : null}
    </div>
  )
}
