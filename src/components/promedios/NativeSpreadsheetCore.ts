export type Primitive = string | number | boolean

export type CellStyle = {
  fontName?: string
  fontSize?: number
  bold?: boolean
  italic?: boolean
  underline?: boolean
  background?: string
  color?: string
  horizontal?: 'left' | 'center' | 'right'
  vertical?: 'top' | 'middle' | 'bottom'
  wrap?: boolean
  border?: boolean
  numberFormat?: string
}

export type GridCell = {
  value: Primitive
  formula?: string
  cached?: Primitive
  style?: CellStyle
  hyperlink?: string
  image?: string
}

export type MergeRange = { startRow: number; startCol: number; endRow: number; endCol: number }
export type ChartKind = 'bar' | 'line' | 'pie'
export type ChartSpec = { id: string; kind: ChartKind; title: string; range: MergeRange }

export type SheetData = {
  name: string
  cells: GridCell[][]
  merges: MergeRange[]
  columnWidths: number[]
  rowHeights: number[]
  orientation: 'portrait' | 'landscape'
  showGridlines: boolean
  charts: ChartSpec[]
}

export type WorkbookData = { sheets: SheetData[] }

type ExcelCell = {
  value: unknown
  formula?: string
  result?: unknown
  font?: { name?: string; size?: number; bold?: boolean; italic?: boolean; underline?: boolean; color?: { argb?: string } }
  fill?: { type?: string; pattern?: string; fgColor?: { argb?: string } }
  alignment?: { horizontal?: string; vertical?: string; wrapText?: boolean }
  border?: Record<string, { style?: string }>
  numFmt?: string
}

type ExcelColumn = { width?: number }
type ExcelRow = { height?: number }
type ExcelWorksheet = {
  name: string
  state?: string
  actualRowCount: number
  actualColumnCount: number
  model?: { merges?: string[] }
  getCell: (row: number, col: number) => ExcelCell
  getRow: (row: number) => ExcelRow
  columns: ExcelColumn[]
  mergeCells: (range: string) => void
  addImage?: (imageId: number, range: { tl: { col: number; row: number }; ext: { width: number; height: number } }) => void
}

type ExcelWorkbook = {
  worksheets: ExcelWorksheet[]
  addWorksheet: (name: string) => ExcelWorksheet
  addImage?: (options: { base64: string; extension: 'png' | 'jpeg' | 'gif' }) => number
  xlsx: { load: (buffer: ArrayBuffer) => Promise<void>; writeBuffer: () => Promise<ArrayBuffer> }
}

type BrowserWindow = Window & {
  ExcelJS?: { Workbook: new () => ExcelWorkbook }
  QRCode?: new (element: HTMLElement, options: { text: string; width: number; height: number }) => unknown
}

export const DEFAULT_ROWS = 40
export const DEFAULT_COLS = 10
export const MAX_ROWS = 800
export const MAX_COLS = 100
const EXCELJS_SRC = 'https://cdn.jsdelivr.net/npm/exceljs@4.4.0/dist/exceljs.min.js'
const QRCODE_SRC = 'https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js'
const META_SHEET = '__PROMEDIOS_META__'

export function emptyCell(): GridCell { return { value: '' } }

export function emptySheet(name = 'Hoja 1', rows = DEFAULT_ROWS, cols = DEFAULT_COLS): SheetData {
  return {
    name,
    cells: Array.from({ length: rows }, () => Array.from({ length: cols }, emptyCell)),
    merges: [],
    columnWidths: Array.from({ length: cols }, (_, index) => index === 0 ? 24 : 14),
    rowHeights: Array.from({ length: rows }, () => 22),
    orientation: 'portrait',
    showGridlines: true,
    charts: [],
  }
}

export function columnLabel(index: number) {
  let value = index + 1
  let result = ''
  while (value > 0) {
    const remainder = (value - 1) % 26
    result = String.fromCharCode(65 + remainder) + result
    value = Math.floor((value - 1) / 26)
  }
  return result
}

function parseCellRef(ref: string) {
  const match = ref.replace(/\$/g, '').match(/^([A-Z]+)(\d+)$/i)
  if (!match) return null
  let col = 0
  for (const char of match[1].toUpperCase()) col = col * 26 + char.charCodeAt(0) - 64
  return { row: Number(match[2]) - 1, col: col - 1 }
}

export function parseRange(range: string): MergeRange | null {
  const [a, b = a] = range.split(':')
  const start = parseCellRef(a)
  const end = parseCellRef(b)
  if (!start || !end) return null
  return {
    startRow: Math.min(start.row, end.row),
    startCol: Math.min(start.col, end.col),
    endRow: Math.max(start.row, end.row),
    endCol: Math.max(start.col, end.col),
  }
}

export function rangeAddress(range: MergeRange) {
  return `${columnLabel(range.startCol)}${range.startRow + 1}:${columnLabel(range.endCol)}${range.endRow + 1}`
}

export function normalizeRange(a: { row: number; col: number }, b: { row: number; col: number }): MergeRange {
  return {
    startRow: Math.min(a.row, b.row),
    startCol: Math.min(a.col, b.col),
    endRow: Math.max(a.row, b.row),
    endCol: Math.max(a.col, b.col),
  }
}

export function mergeAt(merges: MergeRange[], row: number, col: number) {
  return merges.find((item) => row >= item.startRow && row <= item.endRow && col >= item.startCol && col <= item.endCol) ?? null
}

function toPrimitive(value: unknown): Primitive {
  if (typeof value === 'number' || typeof value === 'string' || typeof value === 'boolean') return value
  if (value instanceof Date) return value.toLocaleDateString('es-CL')
  if (value && typeof value === 'object') {
    const objectValue = value as { text?: unknown; richText?: Array<{ text?: unknown }>; result?: unknown }
    if (objectValue.richText) return objectValue.richText.map((item) => String(item.text ?? '')).join('')
    if ('text' in objectValue) return String(objectValue.text ?? '')
    if ('result' in objectValue) return toPrimitive(objectValue.result)
  }
  return value == null ? '' : String(value)
}

function normalizeArgb(argb?: string) {
  if (!argb) return undefined
  const clean = argb.replace(/^#/, '')
  if (clean.length === 8) return `#${clean.slice(2)}`
  if (clean.length === 6) return `#${clean}`
  return undefined
}

function fromExcelCell(cell: ExcelCell): GridCell {
  const style: CellStyle = {
    fontName: cell.font?.name,
    fontSize: cell.font?.size,
    bold: cell.font?.bold,
    italic: cell.font?.italic,
    underline: Boolean(cell.font?.underline),
    color: normalizeArgb(cell.font?.color?.argb),
    background: normalizeArgb(cell.fill?.fgColor?.argb),
    horizontal: ['left', 'center', 'right'].includes(String(cell.alignment?.horizontal)) ? cell.alignment?.horizontal as CellStyle['horizontal'] : undefined,
    vertical: ['top', 'middle', 'bottom'].includes(String(cell.alignment?.vertical)) ? cell.alignment?.vertical as CellStyle['vertical'] : undefined,
    wrap: cell.alignment?.wrapText,
    border: Boolean(cell.border && Object.keys(cell.border).length),
    numberFormat: cell.numFmt,
  }
  const objectValue = cell.value && typeof cell.value === 'object' ? cell.value as { hyperlink?: unknown } : null
  const hyperlink = typeof objectValue?.hyperlink === 'string' ? objectValue.hyperlink : undefined
  if (cell.formula) return { value: `=${cell.formula}`, formula: cell.formula, cached: cell.result == null ? undefined : toPrimitive(cell.result), style, hyperlink }
  return { value: toPrimitive(cell.value), style, hyperlink }
}

export function toNumber(value: Primitive | undefined) {
  if (typeof value === 'number') return value
  if (typeof value === 'boolean') return value ? 1 : 0
  const text = String(value ?? '').trim()
  if (!text) return 0
  const parsed = Number(text.replace(/%$/, '').replace(',', '.'))
  if (!Number.isFinite(parsed)) return 0
  return text.endsWith('%') ? parsed / 100 : parsed
}

export function isNumeric(value: Primitive | undefined) {
  if (typeof value === 'number') return true
  if (typeof value === 'boolean') return false
  const text = String(value ?? '').trim()
  return text !== '' && Number.isFinite(Number(text.replace(/%$/, '').replace(',', '.')))
}

function compareValues(left: Primitive | undefined, right: Primitive | undefined, operator: string) {
  if (isNumeric(left) && isNumeric(right)) {
    const a = toNumber(left)
    const b = toNumber(right)
    if (operator === '>=') return a >= b
    if (operator === '<=') return a <= b
    if (operator === '<>') return a !== b
    if (operator === '>') return a > b
    if (operator === '<') return a < b
    return a === b
  }
  const a = String(left ?? '')
  const b = String(right ?? '')
  if (operator === '>=') return a >= b
  if (operator === '<=') return a <= b
  if (operator === '<>') return a !== b
  if (operator === '>') return a > b
  if (operator === '<') return a < b
  return a === b
}

function notaChile(percent: number, exigencia = 60) {
  const p = Math.max(0, Math.min(100, percent))
  const e = Math.max(1, Math.min(99, exigencia))
  const grade = p <= e ? 1 + (p / e) * 3 : 4 + ((p - e) / (100 - e)) * 3
  return Math.max(1, Math.min(7, Math.round(grade * 10) / 10))
}

function splitArgs(input: string) {
  const args: string[] = []
  let current = ''
  let depth = 0
  let quote = ''
  for (let index = 0; index < input.length; index += 1) {
    const char = input[index]
    if ((char === '"' || char === "'") && input[index - 1] !== '\\') {
      if (!quote) quote = char
      else if (quote === char) quote = ''
      current += char
      continue
    }
    if (!quote) {
      if (char === '(') depth += 1
      else if (char === ')') depth -= 1
      else if (depth === 0 && (char === ',' || char === ';')) {
        args.push(current.trim())
        current = ''
        continue
      }
    }
    current += char
  }
  args.push(current.trim())
  return args
}

function stripOuterParens(value: string) {
  let expression = value.trim()
  let changed = true
  while (changed && expression.startsWith('(') && expression.endsWith(')')) {
    changed = false
    let depth = 0
    let quote = ''
    for (let index = 0; index < expression.length; index += 1) {
      const char = expression[index]
      if ((char === '"' || char === "'") && expression[index - 1] !== '\\') {
        if (!quote) quote = char
        else if (quote === char) quote = ''
      } else if (!quote) {
        if (char === '(') depth += 1
        else if (char === ')') depth -= 1
        if (depth === 0 && index < expression.length - 1) return expression
      }
    }
    if (depth === 0) {
      expression = expression.slice(1, -1).trim()
      changed = true
    }
  }
  return expression
}

function findComparison(expression: string) {
  let depth = 0
  let quote = ''
  for (let index = 0; index < expression.length; index += 1) {
    const char = expression[index]
    if ((char === '"' || char === "'") && expression[index - 1] !== '\\') {
      if (!quote) quote = char
      else if (quote === char) quote = ''
      continue
    }
    if (quote) continue
    if (char === '(') { depth += 1; continue }
    if (char === ')') { depth -= 1; continue }
    if (depth !== 0) continue
    const two = expression.slice(index, index + 2)
    if (['>=', '<=', '<>'].includes(two)) return { index, operator: two }
    if (['=', '>', '<'].includes(char)) return { index, operator: char }
  }
  return null
}

function findBinary(expression: string, operators: string[]) {
  let depth = 0
  let quote = ''
  let candidate: { index: number; operator: string } | null = null
  for (let index = 0; index < expression.length; index += 1) {
    const char = expression[index]
    if ((char === '"' || char === "'") && expression[index - 1] !== '\\') {
      if (!quote) quote = char
      else if (quote === char) quote = ''
      continue
    }
    if (quote) continue
    if (char === '(') { depth += 1; continue }
    if (char === ')') { depth -= 1; continue }
    if (depth !== 0 || !operators.includes(char)) continue
    if ((char === '+' || char === '-') && (index === 0 || '+-*/^(,<>=;'.includes(expression[index - 1]))) continue
    candidate = { index, operator: char }
  }
  return candidate
}

function parseWholeFunction(expression: string) {
  const match = expression.match(/^([A-ZÁÉÍÓÚ.]+)\s*\(/i)
  if (!match) return null
  const open = expression.indexOf('(', match[1].length)
  let depth = 0
  let quote = ''
  for (let index = open; index < expression.length; index += 1) {
    const char = expression[index]
    if ((char === '"' || char === "'") && expression[index - 1] !== '\\') {
      if (!quote) quote = char
      else if (quote === char) quote = ''
      continue
    }
    if (quote) continue
    if (char === '(') depth += 1
    else if (char === ')') {
      depth -= 1
      if (depth === 0) {
        if (index !== expression.length - 1) return null
        return { name: match[1].toUpperCase(), args: splitArgs(expression.slice(open + 1, index)) }
      }
    }
  }
  return null
}

function rangeValues(sheet: SheetData, token: string, visited: Set<string>) {
  const parsed = parseRange(stripOuterParens(token.replace(/\$/g, '')))
  if (!parsed) return []
  const values: Primitive[] = []
  for (let row = parsed.startRow; row <= parsed.endRow; row += 1) {
    for (let col = parsed.startCol; col <= parsed.endCol; col += 1) values.push(computeCell(sheet, row, col, new Set(visited)))
  }
  return values
}

function evaluateExpression(sheet: SheetData, raw: string, visited: Set<string>): Primitive | undefined {
  const expression = stripOuterParens(raw.trim().replace(/^=/, ''))
  if (!expression) return ''
  if ((expression.startsWith('"') && expression.endsWith('"')) || (expression.startsWith("'") && expression.endsWith("'"))) return expression.slice(1, -1)
  if (/^-?\d+(?:[.,]\d+)?%?$/.test(expression)) return toNumber(expression)

  const comparison = findComparison(expression)
  if (comparison) {
    const left = evaluateExpression(sheet, expression.slice(0, comparison.index), new Set(visited))
    const right = evaluateExpression(sheet, expression.slice(comparison.index + comparison.operator.length), new Set(visited))
    return compareValues(left, right, comparison.operator)
  }

  const addSub = findBinary(expression, ['+', '-'])
  if (addSub) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, addSub.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(addSub.index + 1), new Set(visited)))
    return addSub.operator === '+' ? left + right : left - right
  }

  const mulDiv = findBinary(expression, ['*', '/'])
  if (mulDiv) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, mulDiv.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(mulDiv.index + 1), new Set(visited)))
    if (mulDiv.operator === '/' && right === 0) return undefined
    return mulDiv.operator === '*' ? left * right : left / right
  }

  const power = findBinary(expression, ['^'])
  if (power) {
    const left = toNumber(evaluateExpression(sheet, expression.slice(0, power.index), new Set(visited)))
    const right = toNumber(evaluateExpression(sheet, expression.slice(power.index + 1), new Set(visited)))
    return left ** right
  }

  if (expression.startsWith('+')) return toNumber(evaluateExpression(sheet, expression.slice(1), new Set(visited)))
  if (expression.startsWith('-')) return -toNumber(evaluateExpression(sheet, expression.slice(1), new Set(visited)))

  const ref = parseCellRef(expression)
  if (ref) return computeCell(sheet, ref.row, ref.col, new Set(visited))

  const call = parseWholeFunction(expression)
  if (!call) return undefined
  const { name, args } = call

  if (name === 'SI' || name === 'IF') {
    const condition = evaluateExpression(sheet, args[0] ?? '', new Set(visited))
    return evaluateExpression(sheet, Boolean(condition) ? args[1] ?? '""' : args[2] ?? '""', new Set(visited))
  }
  if (name === 'SI.ERROR' || name === 'IFERROR') {
    const result = evaluateExpression(sheet, args[0] ?? '', new Set(visited))
    return result === undefined ? evaluateExpression(sheet, args[1] ?? '""', new Set(visited)) : result
  }
  if (name === 'Y' || name === 'AND') return args.every((arg) => Boolean(evaluateExpression(sheet, arg, new Set(visited))))
  if (name === 'O' || name === 'OR') return args.some((arg) => Boolean(evaluateExpression(sheet, arg, new Set(visited))))
  if (name === 'NOTA') return notaChile(toNumber(evaluateExpression(sheet, args[0] ?? '0', new Set(visited))))
  if (name === 'REDONDEAR' || name === 'ROUND') {
    const value = toNumber(evaluateExpression(sheet, args[0] ?? '0', new Set(visited)))
    const digits = Math.max(0, Math.min(8, Math.round(toNumber(evaluateExpression(sheet, args[1] ?? '0', new Set(visited))))))
    const factor = 10 ** digits
    return Math.round(value * factor) / factor
  }
  if (name === 'SUMAPRODUCTO' || name === 'SUMPRODUCT') {
    const ranges = args.map((arg) => rangeValues(sheet, arg, visited))
    if (!ranges.length || ranges.some((range) => range.length !== ranges[0].length)) return undefined
    return ranges[0].reduce((sum, _, index) => sum + ranges.reduce((product, range) => product * toNumber(range[index]), 1), 0)
  }
  if (name === 'CONTAR.SI' || name === 'COUNTIF') {
    const values = rangeValues(sheet, args[0] ?? '', visited)
    const criterion = String(evaluateExpression(sheet, args[1] ?? '""', new Set(visited)) ?? '')
    const criterionMatch = criterion.match(/^(>=|<=|<>|=|>|<)?\s*(.*)$/)
    const operator = criterionMatch?.[1] || '='
    const target = criterionMatch?.[2] ?? criterion
    return values.filter((value) => compareValues(value, target, operator)).length
  }

  const values = args.flatMap((arg) => {
    const clean = stripOuterParens(arg)
    return clean.includes(':') && parseRange(clean.replace(/\$/g, '')) ? rangeValues(sheet, clean, visited) : [evaluateExpression(sheet, clean, new Set(visited))]
  }).filter((value): value is Primitive => value !== undefined)
  const numbers = values.filter(isNumeric).map(toNumber)

  if (name === 'SUMA' || name === 'SUM') return numbers.reduce((sum, value) => sum + value, 0)
  if (name === 'PROMEDIO' || name === 'AVERAGE') return numbers.length ? numbers.reduce((sum, value) => sum + value, 0) / numbers.length : 0
  if (name === 'MAX') return numbers.length ? Math.max(...numbers) : 0
  if (name === 'MIN') return numbers.length ? Math.min(...numbers) : 0
  if (name === 'CONTAR' || name === 'COUNT') return numbers.length
  if (name === 'CONTARA' || name === 'COUNTA') return values.filter((value) => String(value ?? '').trim() !== '').length
  if (name === 'ABS') return Math.abs(toNumber(values[0]))
  return undefined
}

export function computeCell(sheet: SheetData, row: number, col: number, visited = new Set<string>()): Primitive {
  const key = `${row}:${col}`
  if (visited.has(key)) return '#CICLO'
  visited.add(key)
  const cell = sheet.cells[row]?.[col]
  if (!cell) return ''
  if (!cell.formula && !(typeof cell.value === 'string' && cell.value.startsWith('='))) return cell.value
  const formula = cell.formula || String(cell.value).slice(1)
  const calculated = evaluateExpression(sheet, formula, visited)
  if (calculated !== undefined) return typeof calculated === 'number' ? Math.round(calculated * 1000000) / 1000000 : calculated
  if (cell.cached !== undefined) return cell.cached
  return `=${formula}`
}

export function formatDisplay(cell: GridCell, computed: Primitive) {
  if (typeof computed === 'number' && cell.style?.numberFormat?.includes('%')) return `${Math.round(computed * 10000) / 100}%`
  return String(computed ?? '')
}

async function ensureScript(src: string, test: () => boolean, errorMessage: string) {
  if (test()) return
  await new Promise<void>((resolve, reject) => {
    const existing = document.querySelector(`script[src="${src}"]`) as HTMLScriptElement | null
    if (existing) {
      if (test()) return resolve()
      existing.addEventListener('load', () => resolve(), { once: true })
      existing.addEventListener('error', () => reject(new Error(errorMessage)), { once: true })
      return
    }
    const script = document.createElement('script')
    script.src = src
    script.async = true
    script.onload = () => resolve()
    script.onerror = () => reject(new Error(errorMessage))
    document.head.appendChild(script)
  })
}

async function ensureExcelJS() {
  const browser = window as BrowserWindow
  await ensureScript(EXCELJS_SRC, () => Boolean(browser.ExcelJS), 'No se pudo cargar el motor XLSX')
  if (!browser.ExcelJS) throw new Error('Motor XLSX no disponible')
  return browser.ExcelJS
}

function excelFormula(raw: string) {
  let formula = raw.replace(/^=/, '')
  formula = formula
    .replace(/\bSUMA\(/gi, 'SUM(').replace(/\bPROMEDIO\(/gi, 'AVERAGE(')
    .replace(/\bCONTAR\.SI\(/gi, 'COUNTIF(').replace(/\bCONTARA\(/gi, 'COUNTA(').replace(/\bCONTAR\(/gi, 'COUNT(')
    .replace(/\bSUMAPRODUCTO\(/gi, 'SUMPRODUCT(').replace(/\bREDONDEAR\(/gi, 'ROUND(')
    .replace(/\bSI\.ERROR\(/gi, 'IFERROR(').replace(/\bSI\(/gi, 'IF(')
    .replace(/\bY\(/gi, 'AND(').replace(/\bO\(/gi, 'OR(').replace(/;/g, ',')
  if (/^NOTA\(([^)]+)\)$/i.test(formula)) {
    const ref = formula.match(/^NOTA\(([^)]+)\)$/i)?.[1] || 'A1'
    return `IF(${ref}<=60,1+(${ref}/60)*3,4+((${ref}-60)/40)*3)`
  }
  return formula
}

function applyExcelStyle(cell: ExcelCell, style?: CellStyle) {
  if (!style) return
  cell.font = {
    name: style.fontName,
    size: style.fontSize,
    bold: style.bold,
    italic: style.italic,
    underline: style.underline,
    color: style.color ? { argb: `FF${style.color.replace('#', '').toUpperCase()}` } : undefined,
  }
  if (style.background) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${style.background.replace('#', '').toUpperCase()}` } }
  cell.alignment = { horizontal: style.horizontal, vertical: style.vertical, wrapText: style.wrap }
  if (style.border) cell.border = { top: { style: 'thin' }, right: { style: 'thin' }, bottom: { style: 'thin' }, left: { style: 'thin' } }
  if (style.numberFormat) cell.numFmt = style.numberFormat
}

function imageExtension(dataUrl: string): 'png' | 'jpeg' | 'gif' {
  if (dataUrl.startsWith('data:image/jpeg')) return 'jpeg'
  if (dataUrl.startsWith('data:image/gif')) return 'gif'
  return 'png'
}

export async function loadWorkbookFromBuffer(buffer: ArrayBuffer): Promise<WorkbookData> {
  const ExcelJS = await ensureExcelJS()
  const xlsx = new ExcelJS.Workbook()
  await xlsx.xlsx.load(buffer)
  type Meta = { sheets?: Array<{ name: string; orientation?: 'portrait' | 'landscape'; showGridlines?: boolean; charts?: ChartSpec[]; extras?: Array<{ row: number; col: number; hyperlink?: string; image?: string }> }> }
  let meta: Meta = {}
  const metaSheet = xlsx.worksheets.find((ws) => ws.name === META_SHEET)
  if (metaSheet) {
    try { meta = JSON.parse(String(metaSheet.getCell(1, 1).value ?? '{}')) as Meta } catch { meta = {} }
  }

  const sheets = xlsx.worksheets.filter((ws) => ws.name !== META_SHEET).map((ws, sheetIndex) => {
    const merges = (ws.model?.merges ?? []).map(parseRange).filter((item): item is MergeRange => Boolean(item))
    const mergedLastRow = merges.reduce((max, item) => Math.max(max, item.endRow + 1), 0)
    const mergedLastCol = merges.reduce((max, item) => Math.max(max, item.endCol + 1), 0)
    const rows = Math.max(DEFAULT_ROWS, Math.min(MAX_ROWS, Math.max(ws.actualRowCount || 0, mergedLastRow)))
    const cols = Math.max(DEFAULT_COLS, Math.min(MAX_COLS, Math.max(ws.actualColumnCount || 0, mergedLastCol)))
    const cells = Array.from({ length: rows }, (_, row) => Array.from({ length: cols }, (_, col) => fromExcelCell(ws.getCell(row + 1, col + 1))))
    const metaInfo = meta.sheets?.find((item) => item.name === ws.name)
    metaInfo?.extras?.forEach((extra) => {
      if (cells[extra.row]?.[extra.col]) cells[extra.row][extra.col] = { ...cells[extra.row][extra.col], hyperlink: extra.hyperlink, image: extra.image }
    })
    return {
      name: ws.name || `Hoja ${sheetIndex + 1}`,
      cells,
      merges,
      columnWidths: Array.from({ length: cols }, (_, col) => Math.max(8, Math.min(50, ws.columns[col]?.width ?? (col === 0 ? 24 : 14)))),
      rowHeights: Array.from({ length: rows }, (_, row) => Math.max(18, Math.min(120, ws.getRow(row + 1).height ?? 22))),
      orientation: metaInfo?.orientation ?? 'portrait',
      showGridlines: metaInfo?.showGridlines ?? true,
      charts: metaInfo?.charts ?? [],
    } satisfies SheetData
  })

  return { sheets: sheets.length ? sheets : [emptySheet()] }
}

export async function buildWorkbookBuffer(workbook: WorkbookData) {
  const ExcelJS = await ensureExcelJS()
  const xlsx = new ExcelJS.Workbook()
  const metadata = {
    sheets: workbook.sheets.map((source) => ({
      name: source.name,
      orientation: source.orientation,
      showGridlines: source.showGridlines,
      charts: source.charts,
      extras: source.cells.flatMap((row, rowIndex) => row.flatMap((cell, colIndex) => cell.hyperlink || cell.image ? [{ row: rowIndex, col: colIndex, hyperlink: cell.hyperlink, image: cell.image }] : [])),
    })),
  }

  workbook.sheets.forEach((source, sheetIndex) => {
    const safeName = (source.name || `Hoja ${sheetIndex + 1}`).replace(/[\\/?*:[\]]/g, '-').slice(0, 31)
    const ws = xlsx.addWorksheet(safeName || `Hoja ${sheetIndex + 1}`)
    source.cells.forEach((row, rowIndex) => row.forEach((gridCell, colIndex) => {
      if (gridCell.value === '' && !gridCell.formula && !gridCell.style && !gridCell.image && !gridCell.hyperlink) return
      const cell = ws.getCell(rowIndex + 1, colIndex + 1)
      if (gridCell.formula || (typeof gridCell.value === 'string' && gridCell.value.startsWith('='))) {
        const formula = excelFormula(gridCell.formula || String(gridCell.value).slice(1))
        const currentResult = computeCell(source, rowIndex, colIndex)
        const result = typeof currentResult === 'string' && currentResult.startsWith('=') ? gridCell.cached : currentResult
        cell.value = result === undefined ? { formula } : { formula, result }
      } else if (gridCell.hyperlink) {
        cell.value = { text: String(gridCell.value || gridCell.hyperlink), hyperlink: gridCell.hyperlink }
      } else cell.value = gridCell.value
      applyExcelStyle(cell, gridCell.style)
      if (gridCell.image && xlsx.addImage && ws.addImage) {
        const imageId = xlsx.addImage({ base64: gridCell.image, extension: imageExtension(gridCell.image) })
        ws.addImage(imageId, { tl: { col: colIndex, row: rowIndex }, ext: { width: 140, height: 90 } })
      }
    }))
    source.columnWidths.forEach((width, index) => { if (ws.columns[index]) ws.columns[index].width = width })
    source.rowHeights.forEach((height, index) => { ws.getRow(index + 1).height = height })
    source.merges.forEach((merge) => ws.mergeCells(rangeAddress(merge)))
  })

  const meta = xlsx.addWorksheet(META_SHEET)
  meta.state = 'veryHidden'
  meta.getCell(1, 1).value = JSON.stringify(metadata)
  return xlsx.xlsx.writeBuffer()
}

export async function qrDataUrl(text: string) {
  const browser = window as BrowserWindow
  await ensureScript(QRCODE_SRC, () => Boolean(browser.QRCode), 'No se pudo cargar el generador QR')
  if (!browser.QRCode) throw new Error('Generador QR no disponible')
  const mount = document.createElement('div')
  mount.style.position = 'fixed'
  mount.style.left = '-10000px'
  document.body.appendChild(mount)
  try {
    new browser.QRCode(mount, { text, width: 512, height: 512 })
    await new Promise<void>((resolve) => window.setTimeout(resolve, 10))
    const canvas = mount.querySelector('canvas')
    if (canvas) return canvas.toDataURL('image/png')
    const image = mount.querySelector('img') as HTMLImageElement | null
    if (image?.src) return image.src
    throw new Error('No se pudo generar el QR')
  } finally {
    mount.remove()
  }
}

export function readImage(file: File) {
  return new Promise<string>((resolve, reject) => {
    const reader = new FileReader()
    reader.onload = () => resolve(String(reader.result || ''))
    reader.onerror = () => reject(new Error('No se pudo leer la imagen'))
    reader.readAsDataURL(file)
  })
}

export function downloadBuffer(buffer: ArrayBuffer, filename: string, type: string) {
  const blob = new Blob([buffer], { type })
  const url = URL.createObjectURL(blob)
  const anchor = document.createElement('a')
  anchor.href = url
  anchor.download = filename
  anchor.click()
  window.setTimeout(() => URL.revokeObjectURL(url), 1000)
}
