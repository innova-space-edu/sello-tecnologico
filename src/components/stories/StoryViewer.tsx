...
const EMOJIS: StoryReactionEmoji[] = ['❤️','👏','🔥','🌱','💡','🚀','😂','😮']
...
export default function StoryViewer({ story, hasPrevious, hasNext, onPrevious, onNext, onClose, onChanged }: Props) {
  ...
  const shareUrl = useMemo(() => {
    ...
    return `${window.location.origin}/comunidad?historia=${story.id}`
  }, [story.id])

  useEffect(() => {
    const key = visitorIdentity()
    setVisitorKey(key)
    setVisitorName(window.localStorage.getItem('sello_story_visitor_name') ?? '')
  }, [])
  ...
  }, [hasNext, hasPrevious, itemIndex, onClose, onNext, onPrevious, story.items.length])
  ...
  const react = async (emoji: StoryReactionEmoji) => {
  ...
  const submitComment = async (event: React.FormEvent) => {
    ...
    if (!visitorKey || !comment.trim() || sending) return
    setSending(true)
    window.localStorage.setItem('sello_story_visitor_name', visitorName.trim())
    const response = await fetch(`/api/stories/${story.id}/social`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'comment', visitorKey, visitorName, content: comment }),
    })
    const data = await response.json().catch(() => ({}))
    if (response.ok) {
      setComment('')
      ...
      <div className="mx-auto grid h-full max-w-6xl grid-cols-1 items-center gap-5 p-3 lg:grid-cols-[minmax(0,520px)_380px] lg:p-6">
        <section className="relative mx-auto flex h-[72vh] w-full max-w-[440px] overflow-hidden rounded-[2rem] bg-black shadow-2xl sm:h-[88vh]">
          <div className="absolute inset-x-3 top-3 z-20 flex gap-1">
            {story.items.map((entry, index) => (
              <div key={entry.id} className="h-1 flex-1 overflow-hidden rounded-full bg-white/30">
                <div className="h-full bg-white transition-[width] duration-75" style={{ width: index < itemIndex ? '100%' : index === itemIndex ? `${progress * 100}%` : '0%' }} />
              </div>
            ))}
          </div>
          ...
          <div className="absolute inset-x-4 top-7 z-20 flex items-center justify-between gap-3 rounded-2xl bg-black/20 px-3 py-2 backdrop-blur-sm">
            ...
            <div className="flex gap-2">
              {item.media_type === 'video' && <button type="button" onClick={() => setMuted(value => !value)} className="rounded-full bg-black/40 px-3 py-2 text-xs font-black">{muted ? '🔇' : '🔊'}</button>}
          ...
          {item.media_type === 'video' ? (
            <video ref={videoRef} key={item.id} src={item.signed_url} autoPlay muted={muted} playsInline onTimeUpdate={event => { const target = event.currentTarget; if (target.duration) setProgress(target.currentTime / target.duration) }} onEnded={goForward} className="h-full w-full object-contain" />
          ) : (
          ...
          <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black via-black/65 to-transparent p-5 pt-20">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs font-black text-white/80">
              <span>✨ {reactionCount}</span><span>💬 {commentCount}</span><span>👁 {viewCount}</span>
            </div>
          </div>
        </section>
        ...
        <aside className="hidden max-h-[88vh] overflow-y-auto rounded-[2rem] bg-white p-5 text-slate-900 shadow-2xl lg:block">
          <div className="flex items-start justify-between gap-3">
            <div><h3 className="text-lg font-black">Reacciona y comenta</h3><p className="text-xs text-slate-500">Reconoce el trabajo de la comunidad.</p></div>
            <button type="button" onClick={report} className="text-xs font-bold text-slate-400 hover:text-red-600">Reportar</button>
          </div>

          <div className="mt-4 grid grid-cols-4 gap-2">
            {EMOJIS.map(emoji => (
              <button key={emoji} type="button" disabled={sending} onClick={() => react(emoji)} className={`rounded-2xl border px-2 py-3 text-xl transition hover:-translate-y-0.5 ${viewerReaction === emoji ? 'border-blue-400 bg-blue-50 shadow-sm' : 'border-slate-100 bg-slate-50'}`}>
                <span>{emoji}</span><span className="ml-1 text-[10px] font-black text-slate-500">{reactions[emoji] ?? 0}</span>
              </button>
            ))}
          </div>
          ...
          <div className="relative mt-4">
            ...
            {shareOpen && <div className="mt-2 grid gap-2 rounded-2xl border border-slate-100 bg-white p-3 shadow-xl">
          ...
          <form onSubmit={submitComment} className="mt-5 space-y-2 border-t border-slate-100 pt-5">
            <input value={visitorName} onChange={event => setVisitorName(event.target.value)} placeholder="Tu nombre" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-200" />
            <div className="flex gap-2">
              <textarea value={comment} onChange={event => setComment(event.target.value)} rows={2} placeholder="Escribe un comentario…" className="min-h-12 flex-1 resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-200" />
              <button type="submit" disabled={sending || !comment.trim()} className="self-end rounded-xl bg-blue-600 px-4 py-2 text-xs font-black text-white disabled:bg-slate-300">Publicar</button>
            </div>
          </form>
